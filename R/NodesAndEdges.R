...
NodesAndEdges = function(df, value_bins = 3){
  ...
  nodes = data.frame(id = unique(c(unique(df$from), unique(df$to))),
    ...
    left_join(distinct(df, club, club_icon), by = c("id" = "club")) %>%
    rename(image = club_icon) %>%
    mutate(
  ...
  edges = df %>%
    ...
    left_join(data.frame(value_stage = 1:value_bins, color = viridis::viridis(n = value_bins), stringsAsFactors = FALSE), by = c("value_stage")) %>%
    select(from, to, arrows, dashes, color, title)
